...
|-----------|------------|-----------|
| **Language** | Python 3.12+ | Mature async ecosystem, OpenAI SDK, FastMCP support |
| **Agents Framework** | OpenAI Agents SDK | Native OpenAI model support, tool/function calling |
| **LLM Backend** | vLLM (OpenAI-compatible) | Self-hosted, on-premise, OpenAI API compatibility |
| **Cache / Sessions** | Redis | Token-carrying stateless sessions, rate limiting |
| **Persistent Memory** | PostgreSQL | Structured storage; swap to Mem0 later without API changes |
| **MCP Framework** | FastMCP | Lightweight, Python-native MCP server development |
| **API Framework** | FastAPI | Async, OpenAPI generation, easy REST integration |
| **Observability** | OpenTelemetry | Full distributed tracing, vendor-neutral |
| **Packaging** | Docker Compose (dev), Kubernetes (prod-ready) | Containerization from day one for k8s migration |
| **Vector Search** | Meilisearch | On-premise semantic search capability |
...
services:
  ...
  agent_file:
    build: ./services/agents/file_agent
    environment:
