...
"""
Define a lifespan for MCP Server: 
most MCP server has some sort of client. 
we want to define one client during one life cycle 
"""
@asynccontextmanager
...
mcp = FastMCP(
    ...
    host=os.getenv("HOST", "0.0.0.0"),  # for SSE transport
    port=os.getenv("PORT", "8050")      # for SSE transport
)

"""
Docstring is the description for the tool that is given to LLM so that LLM can decide how and when to use it
Very important to be descriptive!
"""
@mcp.tool()
async def save_memory(ctx: Context, text: str) -> str:
    ...
    try:
        ...
        return f"Successfully saved memory: {text[:100]}..." if len(
            text) > 100 else f"Successfully saved memory: {text}"
